...
#
#   # Monitor every 3 minutes with custom capacity
#   ./monitor-and-deploy.sh --stack-id ocid1.ormstack.oc1... --interval 180 --ocpus 2 --memory-gb 12
#
#   # Limited attempts (100 checks = ~8 hours with 5min interval)
#   ./monitor-and-deploy.sh --stack-id ocid1.ormstack.oc1... --max-attempts 100
#
# Note: This script uses OCI CLI with default or instance principal authentication.
#       No --profile flag needed when running in OCI Cloud Shell or Resource Manager.
###############################################################################
...
if [ -z "$STACK_ID" ]; then
    echo -e "${RED}ERROR: --stack-id is required${NC}" >&2
    echo "Use --help for usage information" >&2
    exit 1
fi
...
STACK_INFO=$(oci resource-manager stack get --stack-id "$STACK_ID" 2>/dev/null || echo "")

if [ -z "$STACK_INFO" ]; then
    echo -e "${RED}ERROR: Unable to retrieve stack information. Check stack ID and credentials.${NC}" >&2
    exit 1
fi
...
if [ -z "$TENANCY_OCID" ]; then
    echo -e "${RED}ERROR: Unable to retrieve tenancy OCID.${NC}" >&2
    exit 1
fi
...
if [ -z "$AVAILABILITY_DOMAINS" ]; then
    echo -e "${RED}ERROR: No availability domains found in region $REGION${NC}" >&2
    exit 1
fi
...
deploy_stack() {
    ...
    if [ -z "$APPLY_JOB" ]; then
        echo -e "${RED}✗ Failed to create apply job${NC}" >&2
        return 1
    fi
    ...
    echo -e "  ${BLUE}oci resource-manager job get --job-id $JOB_ID${NC}"
    echo ""
    echo -e "View logs with:"
    echo -e "  ${BLUE}oci resource-manager job get-job-logs --job-id $JOB_ID${NC}"
    echo ""
